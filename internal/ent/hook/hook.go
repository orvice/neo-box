...
// Mutate calls f(ctx, m).
func (f NocoDBBackupPolicyFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.NocoDBBackupPolicyMutation); ok {
		return f(ctx, mv)
	}
...
// Mutate calls f(ctx, m).
func (f NocoDBSnapshotFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.NocoDBSnapshotMutation); ok {
		return f(ctx, mv)
	}
